...
int main(int argc, char *argv[])
{
	...
	if (stat(bzImage, &st) == 0) {
		...
		printf("bzImage size: %d", file->bzImageSize);
	} else
	...
	//Patch for second stage
	if (stat(second, &st) == 0) {
		tmp = st.st_size;
		file->secondSize = htole32(tmp);
	} else
		ERROR("ERROR reading secondstage\n");
	...
	//patch for secondstage
	while((size = fread(buf,1,BUFSIZ,fsecond))){
		fwrite(buf,1,size,foutput);
	}	
